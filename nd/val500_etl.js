...
var tableProcessor = {
    ...
    GDP: function(element) {
      ...
      if (index > 0) {
        return {
          _id: tableProcessor._quarter2Date(
            element[1].substring(0, index),
            element[1].substr(index + 1)
          ),
    ...
    USDX: function(elements, options) {
      ...
      tmpArr
        ...
        .forEach(function(element) {
          console.log(
            "{0},,,,,,{1}".format(
              new Date(element._id).format("yyyy-MM-dd"),
              element.c
            )
          );
        });
  ...
  serieProcessor = {
    ...
    yearMonthDayPattern:
      "<value xid='(\\d+)'>(\\d{4})年(\\d+)月(\\d+)日</value>",
    yearMonth: function(element) {
      return [
        "_" + element[1],
        myUtil.getLastDateOfMonth(element[2], element[3])
      ];
    },
...
function main(key) {
  ...
  _get(settings.path, settings.file)
    .then(
      function(content) {
        // _save(key, content);

        if (settings.chart) {
          var chartElements = content
            .toString()
            .match(new RegExp(_chartPaser.chartPattern, "gi"));
  ...
  var _chartPaser = {
    ...
    _dateParse: function(chartContent, pattern, processor) {
      return _parse(/<series>(.+?)<\/series>/g, chartContent, function(
        serieElement
      ) {
    ...
    parseYearMonth: function(chartContent) {
      return _chartPaser._dateParse(
        chartContent,
        serieProcessor.yearMonthPattern,
        serieProcessor.yearMonth
      );
    },
    parseYearMonthDay: function(chartContent) {
      return _chartPaser._dateParse(
        chartContent,
        serieProcessor.yearMonthDayPattern,
        serieProcessor.yearMonthDay
      );
    },
    parseNumber: function(chartContent) {
      return _parse(/<graph\s.+?>(.+?)<\/graph>/g, chartContent, function(
        graphElement
      ) {
        return _parse(
          new RegExp(graphProcessor.numberPattern, "g"),
          graphElement[1],
          graphProcessor.parseNumber
        );
      });
    }
  };
