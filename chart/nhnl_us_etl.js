...
function firstRun() {
  ...
  var readFeatureFile = function(market, feature, column, startIndex, rowDic) {
    ...
    var lines = myUtil.readlinesSync("./d/{0}_{1}.txt".format(market, feature));
    if (!lines) return;
...
function dailyRun() {
  ...
  if (process.argv.length == 4) {
    ...
    console.log("USAGE: node nhnl_us_etl.js daily <date with format YYYYMMdd>");
  }
}
...
if (process.argv.length > 2) {
  ...
  if (funcName === "first") {
    firstRun();
    // process.exit(0);
  } else if (funcName === "daily") {
    dailyRun();
    process.exit(0);
  }
}
console.log("something wrong, please choose your action: [first] or [daily]");
console.log("----------------");
